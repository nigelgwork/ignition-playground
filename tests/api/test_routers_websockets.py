...
def test_websocket_endpoint_structure():
    ...
    websocket_paths = [route.path for route in routes if hasattr(route, 'path') and '/ws' in route.path]

    assert len(websocket_paths) > 0, "WebSocket endpoints should be registered"
    assert any('/ws/executions' in path for path in websocket_paths), "Execution WebSocket endpoint should exist"


def test_websocket_managers_initialization():
    """Test that WebSocket connection managers are initialized"""
    from ignition_toolkit.api.routers.websockets import get_websocket_connections, get_active_engines
